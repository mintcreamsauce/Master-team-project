...
def run_etl_pipeline(max_articles: int = 3):
    ...
    # [추가] 파이프라인 완료 후, 세션에 추가된 데이터를 최종 커밋
    try:
        db.session.commit()
        print("✓ Final database session committed successfully!")
    except Exception as e:
        db.session.rollback()
        print(f"✗ ERROR: Final session commit failed (rolling back): {e}")
        error_count += 1
...
if __name__ == "__main__":
    ...
    with app.app_context():
        ...
        run_etl_pipeline(max_articles=10)
